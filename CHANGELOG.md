...
- Split the supplemental UI stylesheet and the runtime script out of `asset/tbb.xml` into standalone files: `asset/src/tbb.v2.2.1.css` and `asset/src/tbb.v2.2.1.js`. `asset/tbb.xml` now loads both externally via jsdelivr (`<link id="tbb-ui-v221" rel="stylesheet">` / `<script id="tbb-runtime-v221" defer src="...">`, same CDN pattern already used for `master-ads-injector.js`), pinned to this branch's commit SHA so the URLs are stable while testing. Once this lands on `main`, repoint both URLs at `@main` (or a tagged release) for auto-updating hosting, or re-pin to a new commit SHA after any further edits to the split files — jsdelivr caches a commit-pinned URL indefinitely, so editing `asset/src/tbb.v2.2.1.{css,js}` alone will not update what a previously-pinned URL serves.
