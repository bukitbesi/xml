...
- Site JS rewritten as a single vanilla-JS runtime (`tbb-runtime-v221`) — no jQuery or other DOM library dependency; verified zero jQuery references in the shipped template and scripts.
...
- Split the supplemental UI stylesheet and the runtime script out of `asset/tbb.xml` into standalone files: `asset/src/tbb.v2.2.1.css` and `asset/src/tbb.v2.2.1.js`. `asset/tbb.xml` now loads both externally via jsdelivr (`<link id="tbb-ui-v221" rel="stylesheet">` / `<script id="tbb-runtime-v221" defer src="...">`, same CDN pattern already used for `master-ads-injector.js`), pointed at `@main` on this repo so any future edit to the split files on `main` picks up automatically on the next Blogger publish (jsdelivr's edge cache for a branch ref refreshes roughly every 12–24h; purge instantly at `https://purge.jsdelivr.net/gh/bukitbesi/xml@main/asset/src/tbb.v2.2.1.js` — and the same for `.css` — if a change needs to show up sooner). Pin to a specific commit SHA instead only when testing an in-progress change that shouldn't affect the live site yet.
