/* The Bukit Besi runtime 2.2.2. Native browser APIs. */
(() => {
  'use strict';
  ...
  function images(root = document) {
    all('.thumbnail[data-src],.avatar[data-src]', root).forEach(el => {
      ...
      if (visible || !('IntersectionObserver' in window)) loadBackground(el, visible && !!el.closest('.featured .first'));
      else {
    ...
    input.addEventListener('input',() => {
      ...
      timer = setTimeout(async () => {
        ...
        try {
          ...
          results.innerHTML = render(posts,'search'); results.classList.add('scroll'); images(results);
